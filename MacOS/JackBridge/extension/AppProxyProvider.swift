...
struct ProxyRule: Codable {
    ...
    private static func matchProcessList(_ processList: String, processPath: String) -> Bool {
        if processList.isEmpty || processList == "*" || processList.uppercased() == "ANY" {
            return true
        }
...
class AppProxyProvider: NETransparentProxyProvider {
    ...
    override func handleAppMessage(_ messageData: Data, completionHandler: ((Data?) -> Void)?) {
        ...
        case "moveRule":
            if let ruleId = message["ruleId"] as? UInt32,
               let newPosition = message["newPosition"] as? Int {
                rulesLock.lock()
                if let currentIndex = rules.firstIndex(where: { $0.ruleId == ruleId }) {
                    let rule = rules.remove(at: currentIndex)
                    let targetIndex = max(0, min(newPosition - 1, rules.count))
                    rules.insert(rule, at: targetIndex)
                    rulesLock.unlock()
                    log("Moved rule #\(ruleId) to priority \(targetIndex + 1)")
                    let response: [String: Any] = ["status": "ok", "ruleId": ruleId, "position": targetIndex + 1]
                    completionHandler?(try? JSONSerialization.data(withJSONObject: response))
                } else {
                    rulesLock.unlock()
                    let response = ["status": "error", "message": "Rule not found"]
                ...
                let response = ["status": "error", "message": "Missing ruleId or newPosition"]
                completionHandler?(try? JSONSerialization.data(withJSONObject: response))
            }
    ...
    private func handleTCPFlow(_ flow: NEAppProxyTCPFlow) -> Bool {
        ...
        if processPath == "com.interceptsuite.JackBridge" || processPath == "com.interceptsuite.JackBridge.extension" {
            return false
        }
