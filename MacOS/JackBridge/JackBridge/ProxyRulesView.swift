...
struct ProxyRulesView: View {
    ...
    private func ruleToDictionary(_ rule: ProxyRule) -> [String: Any] {
        [
            "ruleId": rule.id,
    ...
    private func moveRule(_ rule: ProxyRule, offset: Int) {
        guard let session = viewModel.tunnelSession,
              let currentIndex = rules.firstIndex(where: { $0.id == rule.id }) else { return }

        let targetIndex = max(0, min(currentIndex + offset, rules.count - 1))
        guard targetIndex != currentIndex else { return }

        RuleManager.moveRule(session: session, ruleId: rule.id, newPosition: targetIndex + 1) { [self] success, _ in
            if success {
                DispatchQueue.main.async {
                    let movedRule = rules.remove(at: currentIndex)
                    rules.insert(movedRule, at: targetIndex)
                    RuleManager.saveRulesToUserDefaults(rules.map(ruleToDictionary))
                }
            }
        }
    }
    ...
    private func exportSelectedRules() {
        ...
        savePanel.nameFieldStringValue = "JackBridge-Rules.json"
...
struct RuleEditorView: View {
    ...
    init(viewModel: JackBridgeViewModel, existingRule: ProxyRule? = nil, onSave: @escaping () -> Void) {
