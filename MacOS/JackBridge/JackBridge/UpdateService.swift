...
class UpdateService {
    private let githubApiUrl: String? = nil

    func checkForUpdates() async -> VersionInfo {
        do {
            guard let githubApiUrl, let url = URL(string: githubApiUrl) else {
                return errorVersion("Update checks are not configured for JackBridge.")
            }

            var request = URLRequest(url: url)
            request.setValue("JackBridge-UpdateChecker", forHTTPHeaderField: "User-Agent")
            ...
            let pkgAsset = release.assets.first { asset in
                ...
                (asset.name.lowercased().contains("jackbridge") ||
